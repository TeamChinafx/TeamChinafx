...
api_list = [
    ("https://hk4e-launcher-static.mihoyo.com/hk4e_cn/mdk/launcher/api/resource?channel_id=14&launcher_id=17&key=KAtdSsoQ", "CN.md"),
    ("https://sdk-os-static.mihoyo.com/hk4e_global/mdk/launcher/api/resource?launcher_id=10&key=gcStgarh", "README.md")
]
...
def get_and_write_data(api_url, md_file):
    ...
    if data["retcode"] == 0 and data["message"] == "OK":
        ...
        md_content += "This document is based on the api url provided by miHoYo to get the lastest download informations of GI, such as version number, update time, download link, etc. This document is for reference only and does not represent the official views or positions of miHoYo or hoyoverse.\n\n"
